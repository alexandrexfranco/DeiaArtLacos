...
import { ShoppingBag, Heart, User, Menu, X, LogIn, LogOut } from 'lucide-react';
        ...
        <>
            ...
            >
                <div className="container mx-auto px-6 flex items-center justify-between">
                    ...
                    {/* Logo */}
                    <Link to="/" className={`text-2xl md:text-3xl font-handwritten font-bold transition-all hover:scale-105 ${isScrolled ? 'text-pink-500' : 'text-white drop-shadow-md'
                        }`}>
                        Déia Art Laços
                    </Link>
                            ...
                            <div className="relative group">
                                ...
                                <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-pink-100 py-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all transform origin-top-right z-50">
                                    <div className="px-4 py-2 border-b border-gray-100">
                                        <p className="text-sm font-bold text-gray-800 truncate">{user.display_name || 'Usuário'}</p>
                    ...
                    <div className="flex items-center gap-4 md:hidden">
                        ...
                        <button
                            onClick={() => setIsCartOpen(true)}
                            className={`p-2 hover:scale-110 transition-all relative ${!isScrolled ? 'text-white drop-shadow-md' : 'text-gray-600 hover:text-pink-500'
                                }`}
                        >
                            <ShoppingBag className="w-6 h-6" />
                            {itemCount > 0 && (
                                <span className="absolute top-0 right-0 w-5 h-5 bg-pink-500 text-white text-xs font-bold rounded-full flex items-center justify-center animate-bounce">
                                    {itemCount}
                                </span>
                            )}
                        </button>
                        ...
                        {/* Mobile Menu Button */}
                        <button
                            className={`p-1 ${!isScrolled ? 'text-white drop-shadow-md' : 'text-gray-600'}`}
            ...
            <AnimatePresence>
                {isMobileMenuOpen && (
                    ...
                    >
                        ...
                        <nav className="flex flex-col items-center gap-8 text-xl font-medium text-gray-700">
                            <Link to="/" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-pink-500 transition-colors">Início</Link>
                            <Link to="/loja" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-pink-500 transition-colors">Loja</Link>
                            <Link to="/sobre" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-pink-500 transition-colors">Sobre</Link>
                            <Link to="/contato" onClick={() => setIsMobileMenuOpen(false)} className="hover:text-pink-500 transition-colors">Contato</Link>

                            <div className="w-full h-px bg-pink-100 my-4" />
